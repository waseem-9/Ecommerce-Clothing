// import { useState, useEffect } from "react";
// import axios from "axios";
// import React from "react";

// const MyOrder = () => {
//   const user = JSON.parse(sessionStorage.getItem("active-user"));
//   const [myOrderData, setMyOrderData] = useState([]);

//   useEffect(() => {
//     const getMyOrder = async () => {
//       const myOrder = await retrieveMyOrder();
//       if (myOrder) {
//         console.log("my order data is present :)");

//         setMyOrderData(myOrder.orders);
//       }
//     };

//     getMyOrder();
//   }, []);

//   const retrieveMyOrder = async () => {
//     const response = await axios.get(
//       "http://localhost:8000/api/user/myorder?userId=" + user.id
//     );
//     console.log(response.data);
//     return response.data;
//   };

//   return (
//     <div className="mt-3">
//       <div
//         className="card form-card ms-2 me-2 mb-5 custom-bg"
//         style={{
//           height: "45rem",
//         }}
//       >
//         <div className="card-header text-center bg-color custom-bg-text">
//           <h2>My Orders</h2>
//         </div>
//         <div
//           className="card-body"
//           style={{
//             overflowY: "auto",
//           }}
//         >
//           <div className="table-responsive">
//             <table className="table table-hover custom-bg-text text-center">
//               <thead className="bg-color table-bordered border-color">
//                 <tr>
//                   <th scope="col">Order Id</th>
//                   <th scope="col">Product</th>
//                   <th scope="col">Name</th>
//                   <th scope="col">Description</th>
//                   <th scope="col">Quantity</th>
//                   <th scope="col">Total Price</th>
//                   <th scope="col">Order Date</th>
//                   <th scope="col">Delivery Date</th>
//                   <th scope="col">Delivery Status</th>
//                   <th scope="col">Delivery Person</th>
//                   <th scope="col">Delivery Mobile No</th>
//                 </tr>
//               </thead>
//               <tbody className="text-color">
//                 {myOrderData.map((orderData) => {
//                   return (
//                     <tr>
//                       <td>
//                         <b>{orderData.orderId}</b>
//                       </td>
//                       <td>
//                         <img
//                           src={
//                             "http://localhost:8000/api/product/" +
//                             orderData.productImage
//                           }
//                           class="img-fluid"
//                           alt="product_pic"
//                           style={{
//                             maxWidth: "90px",
//                           }}
//                         />
//                       </td>
//                       <td>
//                         <b>{orderData.productName}</b>
//                       </td>
//                       <td>
//                         <b>{orderData.productDescription}</b>
//                       </td>
//                       <td>
//                         <b>{orderData.quantity}</b>
//                       </td>
//                       <td>
//                         <b>{orderData.totalPrice}</b>
//                       </td>
//                       <td>
//                         <b>{orderData.orderDate}</b>
//                       </td>
//                       <td>
//                         <b>{orderData.deliveryDate}</b>
//                       </td>
//                       <td>
//                         <b>{orderData.deliveryStatus}</b>
//                       </td>
//                       <td>
//                         <b>{orderData.deliveryPersonName}</b>
//                       </td>
//                       <td>
//                         <b>{orderData.deliveryPersonContact}</b>
//                       </td>
//                     </tr>
//                   );
//                 })}
//               </tbody>
//             </table>
//           </div>
//         </div>
//       </div>
//     </div>
//   );
// };

// export default MyOrder;









// import { useState, useEffect } from "react";
// import axios from "axios";
// import React from "react";

// const MyOrder = () => {
//   const user = JSON.parse(sessionStorage.getItem("active-user"));
//   const [myOrderData, setMyOrderData] = useState([]);

//   useEffect(() => {
//     const getMyOrder = async () => {
//       const myOrder = await retrieveMyOrder();
//       if (myOrder) {
//         console.log("my order data is present :)");

//         setMyOrderData(myOrder.orders);
//       }
//     };

//     getMyOrder();
//   }, []);

//   const retrieveMyOrder = async () => {
//     const response = await axios.get(
//       "http://localhost:8000/api/user/myorder?userId=" + user.id
//     );
//     console.log(response.data);
//     return response.data;
//   };

//   return (
//     <div className="momt-3">
//       <div
//         className="mocard moform-card moms-2 mome-2 momb-5 mocustom-bg"
//         style={{
//           height: "45rem",
//         }}
//       >
//         <div className="mocard-header text-center mobg-color mocustom-bg-text">
//           <h2>My Orders</h2>
//         </div>
//         <div
//           className="mocard-body"
//           style={{
//             overflowY: "auto",
//           }}
//         >
//           <div className="motable-responsive">
//             <table className="motable motable-hover mocustom-bg-text text-center">
//               <thead className="mobg-color motable-bordered border-color">
//                 <tr>
//                   <th scope="col">Order Id</th>
//                   <th scope="col">Product</th>
//                   <th scope="col">Name</th>
//                   <th scope="col">Description</th>
//                   <th scope="col">Quantity</th>
//                   <th scope="col">Total Price</th>
//                   <th scope="col">Order Date</th>
//                   <th scope="col">Delivery Date</th>
//                   <th scope="col">Delivery Status</th>
//                   <th scope="col">Delivery Person</th>
//                   <th scope="col">Delivery Mobile No</th>
//                 </tr>
//               </thead>
//               <tbody className="text-color">
//                 {myOrderData.map((orderData) => (
//                   <tr key={orderData.orderId}>
//                     <td>
//                       <b>{orderData.orderId}</b>
//                     </td>
//                     <td>
//                       <img
//                         src={
//                           "http://localhost:8000/api/product/" +
//                           orderData.productImage
//                         }
//                         className="img-fluid"
//                         alt="product_pic"
//                         style={{
//                           maxWidth: "90px",
//                         }}
//                       />
//                     </td>
//                     <td>
//                       <b>{orderData.productName}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.productDescription}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.quantity}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.totalPrice}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.orderDate}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.deliveryDate}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.deliveryStatus}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.deliveryPersonName}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.deliveryPersonContact}</b>
//                     </td>
//                   </tr>
//                 ))}
//               </tbody>
//             </table>
//           </div>
//         </div>
//       </div>
//     </div>
//   );
// };

// export default MyOrder;










// import { useState, useEffect } from "react";
// import axios from "axios";
// import React from "react";
// import './MyOrder.css'; // Import the CSS file

// const MyOrder = () => {
//   const user = JSON.parse(sessionStorage.getItem("active-user"));
//   const [myOrderData, setMyOrderData] = useState([]);

//   useEffect(() => {
//     const getMyOrder = async () => {
//       const myOrder = await retrieveMyOrder();
//       if (myOrder) {
//         console.log("my order data is present :)");

//         setMyOrderData(myOrder.orders);
//       }
//     };

//     getMyOrder();
//   }, []);

//   const retrieveMyOrder = async () => {
//     const response = await axios.get(
//       "http://localhost:8000/api/user/myorder?userId=" + user.id
//     );
//     console.log(response.data);
//     return response.data;
//   };

//   return (
//     <div className="momt-3">
//       <div
//         className="mocard moform-card moms-2 mome-2 momb-5 mocustom-bg"
//         style={{
//           height: "45rem",
//         }}
//       >
//         <div className="mocard-header text-center mobg-color mocustom-bg-text">
//           <h2>My Orders</h2>
//         </div>
//         <div
//           className="mocard-body"
//           style={{
//             overflowY: "auto",
//           }}
//         >
//           <div className="motable-responsive">
//             <table className="motable motable-hover mocustom-bg-text text-center">
//               <thead className="mobg-color motable-bordered border-color">
//                 <tr>
//                   <th scope="col">Order Id</th>
//                   <th scope="col">Product</th>
//                   <th scope="col">Name</th>
//                   <th scope="col">Description</th>
//                   <th scope="col">Quantity</th>
//                   <th scope="col">Total Price</th>
//                   <th scope="col">Order Date</th>
//                   <th scope="col">Delivery Date</th>
//                   <th scope="col">Delivery Status</th>
//                   <th scope="col">Delivery Person</th>
//                   <th scope="col">Delivery Mobile No</th>
//                 </tr>
//               </thead>
//               <tbody className="text-color">
//                 {myOrderData.map((orderData) => (
//                   <tr key={orderData.orderId}>
//                     <td>
//                       <b>{orderData.orderId}</b>
//                     </td>
//                     <td>
//                       <img
//                         src={
//                           "http://localhost:8000/api/product/" +
//                           orderData.productImage
//                         }
//                         className="img-fluid"
//                         alt="product_pic"
//                         style={{
//                           maxWidth: "90px",
//                         }}
//                       />
//                     </td>
//                     <td>
//                       <b>{orderData.productName}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.productDescription}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.quantity}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.totalPrice}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.orderDate}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.deliveryDate}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.deliveryStatus}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.deliveryPersonName}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.deliveryPersonContact}</b>
//                     </td>
//                   </tr>
//                 ))}
//               </tbody>
//             </table>
//           </div>
//         </div>
//       </div>
//     </div>
//   );
// };

// export default MyOrder;







// import { useState, useEffect } from "react";
// import axios from "axios";
// import React from "react";
// import './MyOrder.css'; // Import the CSS file

// const MyOrder = () => {
//   const user = JSON.parse(sessionStorage.getItem("active-user"));
//   const [myOrderData, setMyOrderData] = useState([]);

//   useEffect(() => {
//     const getMyOrder = async () => {
//       const myOrder = await retrieveMyOrder();
//       if (myOrder) {
//         console.log("my order data is present :)");

//         setMyOrderData(myOrder.orders);
//       }
//     };

//     getMyOrder();
//   }, []);

//   const retrieveMyOrder = async () => {
//     const response = await axios.get(
//       "http://localhost:8000/api/user/myorder?userId=" + user.id
//     );
//     console.log(response.data);
//     return response.data;
//   };

//   return (
//     <div className="momt-3">
//       <div
//         className="mocard moform-card moms-2 mome-2 momb-5 mocustom-bg"
//         style={{
//           height: "45rem",
//         }}
//       >
//         <div className="mocard-header">
//           <h2>My Orders</h2>
//         </div>
//         <div
//           className="moform-card"
//           style={{
//             overflowY: "auto",
//           }}
//         >
//           <div className="motable-responsive">
//             <table className="motable motable-hover motable-bordered">
//               <thead>
//                 <tr>
//                   <th scope="col">Order Id</th>
//                   <th scope="col">Product</th>
//                   <th scope="col">Name</th>
//                   <th scope="col">Description</th>
//                   <th scope="col">Quantity</th>
//                   <th scope="col">Total Price</th>
//                   <th scope="col">Order Date</th>
//                   <th scope="col">Delivery Date</th>
//                   <th scope="col">Delivery Status</th>
//                   <th scope="col">Delivery Person</th>
//                   <th scope="col">Delivery Mobile No</th>
//                 </tr>
//               </thead>
//               <tbody>
//                 {myOrderData.map((orderData) => (
//                   <tr key={orderData.orderId}>
//                     <td>
//                       <b>{orderData.orderId}</b>
//                     </td>
//                     <td>
//                       <img
//                         src={
//                           "http://localhost:8000/api/product/" +
//                           orderData.productImage
//                         }
//                         className="img-fluid"
//                         alt="product_pic"
//                         style={{
//                           maxWidth: "90px",
//                         }}
//                       />
//                     </td>
//                     <td>
//                       <b>{orderData.productName}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.productDescription}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.quantity}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.totalPrice}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.orderDate}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.deliveryDate}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.deliveryStatus}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.deliveryPersonName}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.deliveryPersonContact}</b>
//                     </td>
//                   </tr>
//                 ))}
//               </tbody>
//             </table>
//           </div>
//         </div>
//       </div>
//     </div>
//   );
// };

// export default MyOrder;








// import { useState, useEffect } from "react";
// import axios from "axios";
// import React from "react";
// import './MyOrder.css'; // Import the CSS file
// import { FaTruck, FaBoxOpen, FaCheckCircle } from 'react-icons/fa'; // Import icons

// const MyOrder = () => {
//   const user = JSON.parse(sessionStorage.getItem("active-user"));
//   const [myOrderData, setMyOrderData] = useState([]);

//   useEffect(() => {
//     const getMyOrder = async () => {
//       const myOrder = await retrieveMyOrder();
//       if (myOrder) {
//         console.log("my order data is present :)");

//         setMyOrderData(myOrder.orders);
//       }
//     };

//     getMyOrder();
//   }, []);

//   const retrieveMyOrder = async () => {
//     const response = await axios.get(
//       "http://localhost:8000/api/user/myorder?userId=" + user.id
//     );
//     console.log(response.data);
//     return response.data;
//   };

//   return (
//     <div className="momt-3">
//       <div
//         className="mocard moform-card moms-2 mome-2 momb-5 mocustom-bg"
//         style={{
//           height: "45rem",
//         }}
//       >
//         <div className="mocard-header">
//           <h2>My Orders</h2>
//           <FaTruck className="icon-move" style={{ fontSize: '2rem', color: '#007bff' }} />
//         </div>
//         <div
//           className="moform-card"
//           style={{
//             overflowY: "auto",
//           }}
//         >
//           <div className="motable-responsive">
//             <table className="motable motable-hover motable-bordered">
//               <thead>
//                 <tr>
//                   <th scope="col">Order Id</th>
//                   <th scope="col">Product</th>
//                   <th scope="col">Name</th>
//                   <th scope="col">Description</th>
//                   <th scope="col">Quantity</th>
//                   <th scope="col">Total Price</th>
//                   <th scope="col">Order Date</th>
//                   <th scope="col">Delivery Date</th>
//                   <th scope="col">Delivery Status</th>
//                   <th scope="col">Delivery Person</th>
//                   <th scope="col">Delivery Mobile No</th>
//                 </tr>
//               </thead>
//               <tbody className="text-color">
//                 {myOrderData.map((orderData) => (
//                   <tr key={orderData.orderId}>
//                     <td>
//                       <b>{orderData.orderId}</b>
//                     </td>
//                     <td>
//                       <img
//                         src={
//                           "http://localhost:8000/api/product/" +
//                           orderData.productImage
//                         }
//                         className="img-fluid"
//                         alt="product_pic"
//                         style={{
//                           maxWidth: "90px",
//                         }}
//                       />
//                     </td>
//                     <td>
//                       <b>{orderData.productName}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.productDescription}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.quantity}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.totalPrice}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.orderDate}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.deliveryDate}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.deliveryStatus}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.deliveryPersonName}</b>
//                     </td>
//                     <td>
//                       <b>{orderData.deliveryPersonContact}</b>
//                     </td>
//                   </tr>
//                 ))}
//               </tbody>
//             </table>
//           </div>
//         </div>
//       </div>
//     </div>
//   );
// };

// export default MyOrder;







import { useState, useEffect } from "react";
import axios from "axios";
import React from "react";
import './MyOrder.css'; // Import the CSS file
import { FaTruck, FaBoxOpen, FaCheckCircle } from 'react-icons/fa'; // Import icons

const MyOrder = () => {
  const user = JSON.parse(sessionStorage.getItem("active-user"));
  const [myOrderData, setMyOrderData] = useState([]);

  useEffect(() => {
    const getMyOrder = async () => {
      const myOrder = await retrieveMyOrder();
      if (myOrder) {
        console.log("my order data is present :)");

        setMyOrderData(myOrder.orders);
      }
    };

    getMyOrder();
  }, []);

  const retrieveMyOrder = async () => {
    const response = await axios.get(
      "http://localhost:8000/api/user/myorder?userId=" + user.id
    );
    console.log(response.data);
    return response.data;
  };

  return (
    <div className="momt-3">
      <div
        className="mocard moform-card moms-2 mome-2 momb-5 mocustom-bg"
        style={{
          height: "45rem",
          position: 'relative' // Ensure the container has relative positioning for absolute children
        }}
      >
        <div className="mocard-header">
          <h2>My Orders</h2>
          <FaTruck className="icon-move" />
        </div>
        <div
          className="moform-card"
          style={{
            overflowY: "auto",
          }}
        >
          <div className="motable-responsive">
            <table className="motable motable-hover motable-bordered">
              <thead>
                <tr>
                  <th scope="col">Order Id</th>
                  <th scope="col">Product</th>
                  <th scope="col">Name</th>
                  <th scope="col">Description</th>
                  <th scope="col">Quantity</th>
                  <th scope="col">Total Price</th>
                  <th scope="col">Order Date</th>
                  <th scope="col">Delivery Date</th>
                  <th scope="col">Delivery Status</th>
                  <th scope="col">Delivery Person</th>
                  <th scope="col">Delivery Mobile No</th>
                </tr>
              </thead>
              <tbody className="text-color">
                {myOrderData.map((orderData) => (
                  <tr key={orderData.orderId}>
                    <td>
                      <b>{orderData.orderId}</b>
                    </td>
                    <td>
                      <img
                        src={
                          "http://localhost:8000/api/product/" +
                          orderData.productImage
                        }
                        className="img-fluid"
                        alt="product_pic"
                        style={{
                          maxWidth: "90px",
                        }}
                      />
                    </td>
                    <td>
                      <b>{orderData.productName}</b>
                    </td>
                    <td>
                      <b>{orderData.productDescription}</b>
                    </td>
                    <td>
                      <b>{orderData.quantity}</b>
                    </td>
                    <td>
                      <b>{orderData.totalPrice}</b>
                    </td>
                    <td>
                      <b>{orderData.orderDate}</b>
                    </td>
                    <td>
                      <b>{orderData.deliveryDate}</b>
                    </td>
                    <td>
                      <b>{orderData.deliveryStatus}</b>
                    </td>
                    <td>
                      <b>{orderData.deliveryPersonName}</b>
                    </td>
                    <td>
                      <b>{orderData.deliveryPersonContact}</b>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      </div>
    </div>
  );
};

export default MyOrder;
